...
```{r, animation.hook='gifski', fig.cap='Opakované spuštění kuliček v Galtonově boxu'}
N <- 11
for (i in 1:N) {
  vysledek <- rbinom(n = n, size = s, prob = p)
  micek <- vysledek[1]
...
```{r, animation.hook='gifski', fig.cap='Ukázka čekání na tramvaj jako náhodný proces z uniformního rozložení'}
...
Graf \@ref(fig:log-data) zobrazuje histogram logaritmu početu slov ve větě a teoretické PDF $N(2.61, 0.66)$. Jak je vidět takovýto model lépe vystihuje rozptyl v počtu slov. Vzhledem k tomu, že máme pouze 66 pozorování nebude taková náhodná proměnná přesně vystihovat teoretické rozložení. Graf \@ref(fig:density-sample-norm) ukazuje odhadnutou hustotu pravděpodobnosti pro 50 výběrů z $N(2.61, 0.66)$, každý o velikosti $n=$ `r n`. Modrá křivka ukazuje naše data, šedé křivky potom simulované výběry. Je vidět, že několiktakových výběrů, které jsou podobně nahnuté doprava nastalo, což znamená, že taková data nejsou tak nepravděpodobná (pokud pocházejí z $log(s_i) \sim N(2.61, 0.66)$).
```{r log-data, fig.cap='Rozložení logaritmu dat a teoretická PDF N(2.61, 0.66)'}
s_log <- log(pocet_slov)
mu <- mean(s_log)
s <- sd(s_log)
x <- seq(0.01, 5, length.out = 1000)
pdf <- dnorm(x, mean = mu, sd = s)
hist(s_log,
...
```{r density-sample-norm, fig.cap='Příklad hustoty pravděpodobnosti 50 výběrů o velikosti n=66 z N(2.61, 0.66)'}
...
plot(d_data,
  type = "l",
  xlab = "log(s)",
  ylab = "Hustota pravděpodobnosti", main = "",
  col = "#1f77b4",
  xlim = c(0, 5), ylim = c(0, 0.8),
  lwd = 2
)
for (i in 1:S) {
  vyber <- rnorm(n, mean = mu, sd = s)
...
```{r}
q <- log(20)
cdf <- pnorm(q, mean = mu, sd = s, lower.tail = FALSE)
```
