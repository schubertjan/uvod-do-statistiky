...
## Bootstrap
Pojďme se na chvilku vrátit ke grafu \@ref(fig:clv-anim). Na tomto grafu jsme ukázali hypotetickou situaci, kdy jsme věděli jak vypadá rozložení proměnné v celé populaci. Dělali jsme opakovené výběry, abychom ukázali, jak by vypadalo rozložení parametru, kdybychom u každého výběru vypočítali parametr, který nás zajímá. Ukazuje se, že pomocí podobné metody můžeme vypočítat rozložení parametru a to i bez toho, abychom měli data za celou populaci. Místo toho, abychom dělali náhodné výběry z populačních dat, tak budeme dělat **náhodné výběry s opakováním z výběru o velikosti $n$**. Výstupem tohoto výpočtu je vektor parametru, který nás zajímá, o velikosti rovnající se počtu výběrů s opakováním. Je důležité zdůraznit, že přestože počítáme rozložení parametru empiricky, některé ze předpokladů, které jsme zmínili u výpočtu teoretického rozložení, jsou stále platné. Především předpoklad o nezávislosti jednotlivých pozorování a o náhodném výběru z populace (pokud chceme dělat úsudky o celé populaci). 
...
p_hat <- bootstrap(.vyber, pomer, 1e5)
...
## Průměr
Jak jsme uvedli v \@ref(clv-procento), CLV říká, že průměr nezávislých proměnných $\theta$ bude normálně rozložen se směrodatnou chybou rovnou $\frac{s}{\sqrt(n)}$^[Směrodatnou odchylku parametru nazýváme směrodatná chyba, aby nedocházelo k záměně se směrodatnou odchylkou výběru.], tedy $\theta \sim N(\theta, \frac{s}{\sqrt(n)})$. U procent $p$ jsme směrodatnou odchylku počítali jako $\sqrt{p(1-p)}$. U průměru počítáme směrodatnou odchylku jako $s = \sqrt{\frac{1}{n-1}\sum_{i=1}^n (x_i - \overline{x})^2}$, tedy jak jsme si ukazovali v \@ref(desc-vars). Rozložení výběrového průměru náhodné proměnné $x$, tedy bude $$\overline{x} \sim T(\overline{x}, \frac{s}{\sqrt{n}}, n-1)$$Platí přitom, že nezáleží na tom, zda náhodné proměnná pochází z normálního rozložení. CLV se týká průměru této proměnné. Jako příklad si ukážeme hypotetickou situaci, v které budeme vědět populační rozložení náhodné proměnné $x_i$, která pochází z uniformního rozložení $x_i \sim U(-10, 10)$. Provedeme 5000 výběrů o velikosti $n=100$ z této populace, u každého spočítáme průměr a poté zobrazíme rozložení těchto průměrů histogramem.
...
```
Jak je vidět z grafu \@ref(fig:clv-unif) rozložení průměrů z `r S` výběrů je normálně rozloženo. Průměr těchto `r S` průměrů je `r round(mean(prumer), 2)` a směrodatná odchylka je `r round(sd(prumer), 2)`. Porovnejme tyto hodnoty s teoretickým rozložením podle CLV. Z kapitoly o uniformním rozložení \@ref(unif-dist) víme, že průměr se vypočítá jako $\mu = \frac{1}{2}(a+b)$, tedy pro  $U(-10 ,10)$ je to hodnota `r (a+b)/2`. Rozptyl je $\sigma^2 = \frac{1}{12}(b-a)^2$, tedy pro  $U(-10 ,10)$ hodnota `r round((b-a)^2/12, 2)`. Směrodatná chyba odhadu je $\frac{s}{\sqrt{n}}$, tedy `r  round(sqrt(((b-a)^2/12) / n), 2)`. Jak je vidět, parametry normálního rozložení průměru podle CLV se shodují s údaji, které jsme získali při simulaci naší hypotetické situace. 
...
```{r clv-sim-t, fig.cap='Prvních 100 simulací výběrů z t-rozdělení při n=66 a zohlednění nejistoty výběrového průměru a směrodatné odchylky'}
# pocet simulaci
s <- 1e5
...
for (i in 1:100) {
  ...
  lines(density(S[i, ]), col = adjustcolor("black", alpha.f = 0.1))
}
