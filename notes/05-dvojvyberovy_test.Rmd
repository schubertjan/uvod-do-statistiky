...
p_d <- p1 - p2
# směrodatná odchylka rozdilu 
sd_d <- sqrt(s1^2 + s2^2)
# smerodatna chyba rozdilu
se_d <- sd_d / sqrt(n1+n2-2)

x <- seq(-0.008, 0.004, length.out = 1000)
...
x <- seq(-0.006, 0.006, length.out = 1000)
h0 <- 0
...
```{r}
x_1 <- c(rep(1, y1), rep(0, n1-y1))
x_2 <- c(rep(1, y2), rep(0, n2-y2))

for(i in 1:s) {
  vyber_1 <- sample(x_1, size = n1, replace = TRUE)
  vyber_2 <- sample(x_2, size = n2, replace = TRUE)
  p_d_boostrap[i] <- mean(vyber_1) - mean(vyber_2)
}
quantile(p_d_boostrap, probs = c(alpha/2, 1-alpha/2))
```


`r alpha/2` kvantil a `r  1-alpha/2` kvantil jsou [`r i_s[1]`, `r i_s[2]`].
